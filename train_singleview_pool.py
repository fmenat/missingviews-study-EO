...
def main_run(config_file):
    ...
    for r in range(runs):
        ...
        for k in range(kfolds):
            ...
            start_aux = time.time()
            method, trainer = PoolEnsemble_train(train_data, val_data=val_data,run_id=r,fold_id=k,
            ...
            for view_n, values in outputs_tr["views:prediction"].items():
                data_save_tr = DataViews([values], identifiers=train_data["identifiers"], view_names=[f"out_run-{r:02d}_fold-{k:02d}"])
                data_save_tr.save(f"{output_dir_folder}/pred/{data_name}/train/{method_name}_{view_n}", ind_views=True, xarray=False)
                mlf_logger.experiment.log_artifact(run_id_mlflow, f"{output_dir_folder}/pred/{data_name}/train/{method_name}_{view_n}/out_run-{r:02d}_fold-{k:02d}.csv",
            ...
            for view_n, values in outputs_te["views:prediction"].items():
                ...
                mlf_logger.experiment.log_artifact(run_id_mlflow, f"{output_dir_folder}/pred/{data_name}/test/{method_name}_{view_n}/out_run-{r:02d}_fold-{k:02d}.csv",
